...
def filter_by_criteria(cars_data: dict, criteria: tuple):
    year, engine_volume, price = criteria

    car_data_filter_by_criteria = {
        car: (car_color, car_year, car_engine_volume, car_form, car_price)
        for car, (
            (car_color, car_year, car_engine_volume, car_form, car_price)
        ) in cars_data.items()
        if (car_year >= year) and (car_engine_volume >= engine_volume) and (car_price <= price)
    }
    
    print(f"Усього знайдено за крітериямі: рік >= {year},"
          f" об'єм двигуна >= {engine_volume},  ціна <={price} - {len(car_data_filter_by_criteria)} машин")
    print("Машини відсортовані за зростанням ціни")
    print("----------------------------------------")
    for car, (car_color, car_year, car_engine_volume, car_form, car_price) in sorted(
        car_data_filter_by_criteria.items(), key=lambda item: item[1][4]
    ):
        print(car, (car_color, car_year, car_engine_volume, car_form, car_price))
        
    print()
    print(" Перші п'ять машін із відсортованого за зростанням цін списку машин")
    print("----------------------------------------")
...
filter_by_criteria(car_data, search_criteria)
